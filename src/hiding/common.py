# -*- coding: utf-8 -*-
"""
src/hiding/common.py — cấu trúc dữ liệu dùng chung cho PART 4 hiding.
...
Lưu ý : xóa một item đổi tw(T_k) ⇒ đổi W_total ⇒ MỌI ws dịch, kể cả itemset
...
class HidingDB:
    ...
    def __init__(self, D: Dict[str, Set[str]], W: Dict[str, float], track: Iterable = None):
        ...
        # num_cache incremental: num_cache[X] = Σ_{T⊇X} tw(T) cho X ∈ S∪~S.
    ...
    def ws(self, X: Iterable[str]):
        """ws(X) = Σ_{t⊇X} tw_cache[t] / W_total (theo W_total HIỆN TẠI — Lưu ý).
    ...
    def delete(self, v: str, t: str) -> None:
        ...
        self.W_total += tw_new - tw_old       # cập nhật W_total (Lưu ý)
        self.tw_cache[t] = tw_new
        for X in affected:                    # num_cache incremental — (3-case)
            if v in X:
