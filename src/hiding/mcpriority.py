# -*- coding: utf-8 -*-
"""
src/hiding/mcpriority.py — MCPriority (Min-Side-Effect, ).
...
Safe(v,T_k) — FULL-CHECK TOÀN BỘ ~S theo W'_total MỚI:
...
def _tid_num(t):
    """TID tăng dần numeric (). 'T<số>' → số; fallback id_key."""
...
def is_safe(db, v, tk, NS, xi, round3=False) -> bool:
    """
    Safe(v,T_k): FULL-CHECK toàn ~S theo W'_total mới. True ⟺ xóa v khỏi T_k
    ...
    if Wp <= 0:
        return False
    nc = db.num_cache                          # num_cache[ns]=O(1) nếu ns được track
    for ns in NS:
