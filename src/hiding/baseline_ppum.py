# -*- coding: utf-8 -*-
"""
src/hiding/baseline_ppum.py — Baseline PPUM (MSU-MAU + MSU-MIU) cho , FWI-deletion.

Port Algorithm 1/2 của Lin et al. (EAAI 55, 2016) sang tw/ws;
...
RANH GIỚI: KHÔNG Safe()/no-op/né-non-sensitive; KHÔNG nhánh
giảm-quantity (FWI không quantity ⇒ luôn xóa); KHÔNG `select_victim` (baseline có victim-rule
riêng); reuse `HidingDB.delete` (Lưu ý) + `metrics.is_frequent` cho test ws≥ξ. Baseline luôn
...
def run_msu_mau(db, S, xi, round3=False):
    ...
    for X in order:
        ...
        while is_frequent(db.ws(X), xi, round3):                     # thay du>0; test exact ws≥ξ
            ...
            db.delete(v, Tstar)                                      # xóa item (reuse delete — Lưu ý)
