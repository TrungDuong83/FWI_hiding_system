# -*- coding: utf-8 -*-
"""
src/metrics/metrics.py — 4 metric FWI. CHỈ HF, MC, AC, RT.
...
- HF/MC recompute `ws` trên DB đã sanitize qua `common.HidingDB.ws` ⇒ dùng **W_total hiện tại**
  (Lưu ý). `S` đóng băng từ DB gốc; `~S` = FWI(orig) \ S đóng băng từ DB gốc.
- Membership: `ws ≥ ξ` TRỰC TIẾP cho cả golden/calibration (Fraction exact) và production (float64).
  BỎ `round(ws,3)` (gây vùng bóng làm-tròn-lên → mismatch sau hiding). `round3` vô hiệu.
...
def is_frequent(ws_val, xi, round3: bool = False) -> bool:
    ...
    BỎ round(·,3): `round(ws,3) ≥ ξ` tạo vùng bóng [ξ−5e-4, ξ) làm-tròn-LÊN →
