...
Chuẩn, backend Fraction (exact):
  ...
  delete cập nhật W_total đúng (Lưu ý); xóa C∈AC khỏi T1 ⇒ ws(AC) giảm ngặt (A1).
...
def test_delete_updates_wtotal():
    ...
    # 2) W_total khớp recompute-from-scratch trên D đã sửa (Lưu ý)
...
def test_preprocess():
    ...
    # Engine = backend float64 production ⇒ dùng float ξ/weights; phần exact do HidingDB.
