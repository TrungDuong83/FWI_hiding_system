# -*- coding: utf-8 -*-
"""
coordinator/run_coordinator.py — MAIN + SWEEP, 5 method. 

Cell list (fixed configuration):
...
Backend = float64 + round(ws,3)≥ξ. REUSE hiding/metrics/miner/calibrate — KHÔNG sửa logic.
...
SWEEP_EXCLUDE_DS = {"chainstore"}                # exclude chainstore from sweep (round3 collapse at small xi)
...
def boundary_audit(db, universe, xi, Wfrac):
    ...
    nằm trong 1e-6 của ξ mà float lệch exact ⇒ caller (ca thật, hiếm).
    """
    Wtot = Fraction(0)
    twf = {}
...
def git_checkpoint(msg):
    # Kết quả đã được ghi ra results/ (result_*.json + summary.csv) sau mỗi ô.
    # Không cần thao tác gì thêm ở đây.
    return
